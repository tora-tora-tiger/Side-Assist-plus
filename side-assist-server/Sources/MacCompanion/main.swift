...
class MacCompanion: NSObject {
    ...
    func start() {
        ...
        print("📱 Looking for Side Assist mobile service...")
        startBrowsing()
    }
    ...
    func startBrowsing() {
        ...
        print("🔍 Browsing for _sideassist._tcp services...")
        isSearching = true
        
        // Create browser for our service type
        let parameters = NWParameters()
        browser = NWBrowser(for: .bonjourWithTXTRecord(type: "_sideassist._tcp", domain: nil), using: parameters)
        ...
        browser?.browseResultsChangedHandler = { [weak self] results, changes in
            for result in results {
                ...
                case .hostPort(let host, let port):
                    print("🎯 Found Side Assist service at \(host):\(port)")
